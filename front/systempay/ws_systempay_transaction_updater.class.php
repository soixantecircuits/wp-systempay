<?php class WSSystempayTransactionUpdater extends WSSystempayAnalyzer {
	public function updateTransaction() {
		...
		if (!empty($order_id)) :
			...
			if (!empty($to_email)):
				$this->sendMail($order_id,$to_email);
			else :
				_e("Email invalide/vide","ws");
			endif;
		else:
			_e("Numéro de commande vide","ws");
		endif;
	...
	public function sendMail($order_id,$to_email){
		...
		$this->useSwiftMailer($order_id,$to_email,$subject,$content);
	}
	...
	private function useSwiftMailer($order_id,$to_email,$subject,$content){
		...
		if($Mailer->send($Email) == 1){
		    _e("Un e-mail vient de vous être envoyé, merci beaucoup et à bientôt.","ws");
		}
		else {
		   	-e("Erreur lors de l'envoi de l'e-mail.","ws");
		}
	}
	...
	public function get_success_mail(){
		ob_start(); 
		require_once(dirname(__FILE__)."/../../templates/emails_templates/success_email.php");
		$mail=ob_get_clean(); 
		return $mail;
	}
	...
	public function get_error_mail(){
		ob_start(); 
		$order_id = $this->get_or_post("vads_order_id");
		require_once(dirname(__FILE__)."/../../templates/emails_templates/error_email.php");
		$mail=ob_get_clean(); 
		return $mail;
	}
