	...
	public function useFunctionOnInputs($inputs_array,$form_id,$posts,$excludes){
		...
		$is_function = (bool) $input_array["function"];
			if ($is_function) {
				...
				if (method_exists($this->Systempay, $function_name)&&(!$isExcluded))
				{
...
//get the html code of the Confirmation Form
	...
	public function saveTransaction($form_id,$datas,$confirmation_form_id){
		...
		$trans_id=$this->Systempay->systemPay_GetTransId();
		...
		$wpdb->show_errors($insert = $wpdb->insert(
	...
	<?php	//we create the hidden form
		...
		//else we propose to retry or to cancel
			...
			$form_id = $_GET[$this->get_GET_key_confirmation_formid()];
				...
				switch($plateforme) :
					case "systempay" :
						...
						if ($configuration["name"]=="signature"){
							$signature = $this->Systempay->WS_GetSignature($configurations_data,$inputs_data,$order_id,$trans_id);
