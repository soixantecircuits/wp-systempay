...
//Method used to transform the inputs from SQL to an array by colin
	...
	public function getFormDataObjectById($form_id){
			global $wpdb;
			(int)($form_id);
			$form_data = $wpdb->get_row("SELECT * FROM $this->form_table_name WHERE form_id = $form_id");
			return $form_data;
	}
    public function getFormIdByName($name) {
		global $wpdb;
		$form_id = $wpdb->get_row("SELECT form_id FROM $this->form_table_name WHERE form_name = '$name'","ARRAY_N");
		return $form_id[0];
	}
//get the formals inputs datas of the SQL database
	public function getConfigurationsObjectById($form_id){
			global $wpdb;
			(int)($form_id);
			$configurations_data = $wpdb->get_results("SELECT * FROM $this->configurations_table_name WHERE configuration_form_id = $form_id");
...
//get the additionals inputs datas of the SQL database
	public function getAdditionalsInputsObjectById($form_id){
			global $wpdb;
			(int)($form_id);
			$fieldsets = $wpdb->get_results("SELECT DISTINCT input_fieldset FROM $this->inputs_table_name WHERE input_form_id = $form_id ORDER BY input_fieldset;");
			$groupes=array();
			foreach ($fieldsets as $fieldset) {
				$number = $fieldset->input_fieldset;
				$input_get = $wpdb->get_results("SELECT * FROM $this->inputs_table_name WHERE input_form_id = $form_id AND input_fieldset = $number ORDER BY input_order;");
...
//get the form in an array with his ID
	...
	public function getLastGeneralConfigId(){
    	global $wpdb;
		$lastId = $wpdb->get_var($wpdb->prepare( "SELECT generalconfig_id FROM $this->generalconfig_table_name ORDER BY generalconfig_id DESC"));	
		if (empty($lastId)){
			return false;
		}
		return($lastId);
	...
	public function getGeneralConfig(){
		...
		if (!empty($config_id)){
			(int)($config_id);
			return json_decode($wpdb->get_var($wpdb->prepare( "SELECT generalconfig_json FROM $this->generalconfig_table_name WHERE generalconfig_id='$config_id'")));
		}
		return false;
	}
	...
	public function WSConfigExist($form_id){
		global $wpdb;
		(int)($form_id);
		$WSconfig_data = $wpdb->get_var("SELECT WSconfig_id FROM $this->WSconfig_table_name WHERE WSconfig_form_id = $form_id");
	...
	public function getFormWSConfig($form_id){
		global $wpdb;
		(int)($form_id);
		$WSconfig_data = $wpdb->get_var("SELECT WSconfig_json FROM $this->WSconfig_table_name WHERE WSconfig_form_id = $form_id");
