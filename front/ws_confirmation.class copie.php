    ...
    */
    ...
    private function getOutterQuery($configurations_data){
        ...
        foreach ($configurations_data as $element){
            $request = array_push($request[$element['name']]);
        }   

        $supQuery = array_diff(array_flip($request), $_REQUEST);
    ...
    private function add_custom_return_url($configurations_data){
        ...
        //url refused
        $configurations_data[28]["value"] = $this->remove_trailing_slash($configurations_data[28]["value"])."/".$other_request;
        //url cancel
        $configurations_data[29]["value"] = $this->remove_trailing_slash($configurations_data[29]["value"])."/".$other_request;
        //url error
        $configurations_data[30]["value"] = $this->remove_trailing_slash($configurations_data[30]["value"])."/".$other_request;
        //url return (if other not present)
        $configurations_data[31]["value"] = $this->remove_trailing_slash($configurations_data[31]["value"])."/".$other_request;
